...
export default function Weight() {
    ...
    useEffect(() => { if (user) { fetchProfile(); fetchLogs(); } }, [user, chartPeriod]);
    ...
    const saveGoal = async () => {
        if (!user || !targetWeight || !targetDate) { alert('Please enter target weight and date'); return; }
        await supabase.from('profiles').update({ target_weight: parseFloat(targetWeight), target_date: targetDate }).eq('id', user.id);
        alert('Goal saved!');
        fetchProfile();
    };
        ...
        <div style={{ paddingBottom: 16 }}>
            ...
            <div className="card" style={{ margin: '16px 16px' }}>
                ...
                <div className="weight-row">
                    <input className="weight-input" type="number" step="0.1" placeholder="Log weight" value={newWeight} onChange={e => setNewWeight(e.target.value)}
                        onKeyDown={e => e.key === 'Enter' && addWeight()} />
                    <button className="weight-add-btn" onClick={addWeight}>Add</button>
                </div>
            </div>
            ...
            <div className="card" style={{ margin: '0 16px 16px' }}>
                ...
                <div className="goal-row">
                    <div className="goal-col">
                        <div style={{ fontSize: 12, color: 'var(--text2)', marginBottom: 6 }}>Target Weight (kg)</div>
                        <input className="goal-input" type="number" step="0.1" placeholder="70" value={targetWeight} onChange={e => setTargetWeight(e.target.value)} />
                    </div>
                    <div className="goal-col">
                        <div style={{ fontSize: 12, color: 'var(--text2)', marginBottom: 6 }}>Target Date</div>
                        <input className="goal-input" type="date" value={targetDate} onChange={e => setTargetDate(e.target.value)} />
                    </div>
                </div>
                <button className="btn btn-primary" style={{ marginTop: 4 }} onClick={saveGoal}>Save Goal</button>
